...
import { formatTimestampToHumanReadable, getFutureDateTime } from "@/lib/utils";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
      ...
      <div className="w-full mt-4 p-5 sm:p-10">
        {isLoading ? (
          ...
          <div className="w-full">
             ...
             <div className="w-full text-center mb-10">Next scrapping scheduled at <span className="font-semibold ">{formatTimestampToHumanReadable(getFutureDateTime(data[data.length-1].started_at).hoursAfter24.toString())}</span>.</div>
           
           <h3 className="font-semibold  ">Recent Scrapping Logs</h3>
            <Accordion type="single" collapsible>
            {data.map((log, i) => (
               <AccordionItem value={`scrape-log-${i}`}  key={i}>
               <AccordionTrigger className="cursor-pointer"> Scrapped at: {formatTimestampToHumanReadable(log.started_at)}</AccordionTrigger>
               <AccordionContent>
